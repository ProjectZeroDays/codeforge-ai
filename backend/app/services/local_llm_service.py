...
class LocalLLMService:
    ...
    def _get_models_dir(self) -> Path:
        """Get the models directory, respecting environment variable or using defaults."""
        env_path = os.getenv("LOCAL_LLM_MODELS_DIR")
        if env_path:
            return Path(env_path)
        if os.name == "nt":
            return Path.home() / "codeforge_ai" / "models"
        return Path("/home/ubuntu/codeforge_ai/models")
    ...
    async def load_model(self, config: ModelConfig = None) -> Dict[str, Any]:
        ...
        if not HAS_TORCH:
            return {"success": False, "error": "PyTorch is not installed. Install torch to use local LLM features."}
